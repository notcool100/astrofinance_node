...
export const getAllAccounts = async (req: Request, res: Response) => {
	try {
		...
		const where: Prisma.Account_COAWhereInput = {};

		if (type) {
			where.accountType = type as AccountType_COA;
		}
...
export const createAccount = async (req: Request, res: Response) => {
	try {
		...
		const existingAccount = await prisma.account_COA.findFirst({
			where: {
				OR: [{ accountCode }, { name: accountName }],
			},
		});
		...
		if (existingAccount) {
			if (existingAccount.accountCode === accountCode) {
				throw new ApiError(
					409,
					`Account with code '${accountCode}' already exists`,
				);
			} else {
				throw new ApiError(
					409,
		...
		if (
			!Object.values(AccountType_COA).includes(accountType as AccountType_COA)
		) {
...
export const updateAccount = async (req: Request, res: Response) => {
	try {
		const { id } = req.params;
		const { accountName, description, parentAccountId, isActive } = req.body;
...
export const getAccountStructure = async (req: Request, res: Response) => {
	try {
		...
		// Build filter conditions for root level
		const where: Prisma.Account_COAWhereInput = {
			parentId: null,
		};
		...
		const childrenWhere: Prisma.Account_COAWhereInput = {};
		if (type) {
			childrenWhere.accountType = type as AccountType_COA;
		}
		if (active !== undefined) {
			childrenWhere.isActive = active === "true";
		}

		// Get all top-level accounts (no parent) with filtered children
