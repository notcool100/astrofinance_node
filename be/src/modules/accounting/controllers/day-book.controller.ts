...
export const createDayBook = async (req: Request, res: Response) => {
  try {
    ...
    if (existingDayBook) {
      throw new ApiError(400, `A day book already exists for ${transactionDate}`);
    }
...
export const reconcileDayBook = async (req: Request, res: Response) => {
  try {
    ...
    const systemCashBalance = parseFloat(existingDayBook.systemCashBalance.toString());
...
export const getDayBookSummary = async (req: Request, res: Response) => {
  try {
    ...
    const accountTypeSummary: Record<string, { debits: number; credits: number }> = {};
    ...
    journalEntries.forEach(entry => {
      entry.journalEntryLines.forEach(line => {
        ...
        accountTypeSummary[accountType].debits += parseFloat(line.debitAmount.toString());
        accountTypeSummary[accountType].credits += parseFloat(line.creditAmount.toString());
      });
    });
