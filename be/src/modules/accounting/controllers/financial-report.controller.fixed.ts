...
import { format as formatDateFns, format } from 'date-fns';
import fs from 'fs';
  ...
  try {
    ...
    switch (type) {
      case 'balance-sheet':
        ...
        fileName = `balance-sheet-${formatDateFns(asOfDateBS, 'yyyy-MM-dd')}`;
        break;
      ...
      case 'income-statement':
        ...
        fileName = `income-statement-${formatDateFns(startDateIS, 'yyyy-MM-dd')}-to-${formatDateFns(endDateIS, 'yyyy-MM-dd')}`;
        break;
      ...
      case 'trial-balance':
        ...
        fileName = `trial-balance-${formatDateFns(asOfDateTB, 'yyyy-MM-dd')}`;
        break;
      ...
      case 'general-ledger':
        ...
        fileName = account 
          ? `general-ledger-${account.accountCode}-${formatDateFns(startDateGL || new Date(), 'yyyy-MM-dd')}-to-${formatDateFns(endDateGL, 'yyyy-MM-dd')}`
          : `general-ledger-${formatDateFns(startDateGL || new Date(), 'yyyy-MM-dd')}-to-${formatDateFns(endDateGL, 'yyyy-MM-dd')}`;
        break;
    }
...
const generateExcelReport = async (type: string, data: any): Promise<Buffer> => {
  ...
  switch (type) {
    case 'balance-sheet':
      ...
      worksheet.addRow([`As of ${formatDate(data.asOfDate)}`]);
      worksheet.addRow([]);
    ...
    case 'income-statement':
      ...
      worksheet.addRow([`For the period ${formatDate(data.period.startDate)} to ${formatDate(data.period.endDate)}`]);
      worksheet.addRow([]);
    ...
    case 'general-ledger':
      ...
      worksheet.addRow([`For the period ${data.period.startDate ? formatDate(data.period.startDate) : ''} to ${data.period.endDate ? formatDate(data.period.endDate) : ''}`]);
      worksheet.addRow([]);
  ...
  return new Promise((resolve, reject) => {
    ...
    switch (type) {
      case 'balance-sheet':
        ...
        doc.fontSize(12).text(`As of ${formatDate(data.asOfDate)}`, { align: 'center' });
        doc.moveDown(2);
      ...
      case 'income-statement':
        ...
        doc.fontSize(12).text(`For the period ${formatDate(data.period.startDate)} to ${formatDate(data.period.endDate)}`, { align: 'center' });
        doc.moveDown(2);
      ...
      case 'general-ledger':
        ...
        doc.fontSize(12).text(`For the period ${data.period.startDate ? formatDate(data.period.startDate) : ''} to ${data.period.endDate ? formatDate(data.period.endDate) : ''}`, { align: 'center' });
        doc.moveDown(2);
