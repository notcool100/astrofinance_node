...
export const createSmsTemplate = async (req: Request, res: Response) => {
	try {
		...
		// Check if SMS event exists (optional)
		if (smsEventId) {
		...
		if (existingTemplate) {
			throw new ApiError(
				409,
				`SMS template with name '${name}' already exists`,
			);
		}
		...
		const template = await prisma.smsTemplate.create({
			data: {
				...
				content,
				category,
				variables: variables as any, // Store variables as JSON
...
export const updateSmsTemplate = async (req: Request, res: Response) => {
	try {
		const { id } = req.params;
		const { name, content, category, variables, smsEventId, isActive } =
			req.body;
		...
		const updatedTemplate = await prisma.smsTemplate.update({
			...
			data: {
				...
				variables: variables !== undefined ? (variables as any) : undefined,
...
export const sendTestSms = async (req: Request, res: Response) => {
	try {
		const { templateId, phoneNumber, placeholders } = req.body;
