...
export const login = async (req: Request, res: Response) => {
	try {
		...
		// Create audit log
		await createAuditLog(req, "User", user.id, AuditAction.LOGIN, null, {
			id: user.id,
			email: user.email,
		});
...
export const updateProfile = async (req: Request, res: Response) => {
	try {
		...
		const { fullName, contactNumber, address, gender } = req.body;
...
export const logout = async (req: Request, res: Response) => {
	try {
		...
		if (userId) {
			// Create audit log
			await createAuditLog(req, "User", userId, AuditAction.LOGOUT, null, {
				id: userId,
			});
		}
