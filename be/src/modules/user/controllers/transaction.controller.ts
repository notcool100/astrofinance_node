...
import { createTransactionSchema, getTransactionsByAccountSchema, getTransactionByIdSchema, cancelTransactionSchema } from '../validations/transaction.validation';
...
export const createTransaction = async (req: Request, res: Response) => {
  try {
    ...
    // Create a journal entry for the transaction if needed
    // This would typically be handled by a separate service or module
    // For now, we'll just return the transaction result
  ...
  try {
    ...
    switch (transaction.transactionType) {
      ...
      default:
        ...
        reversalType = TransactionType.ADJUSTMENT;
        console.warn(`Unexpected transaction type: ${transaction.transactionType}`);
    }
...
export const getTransactionSummary = async (req: Request, res: Response) => {
  try {
    ...
    const currentMonthTransactions = await prisma.userAccountTransaction.findMany({
      where: {
        accountId,
        transactionDate: {
          gte: startOfMonth,
 ...
 */
export const getAllTransactionsSummary = async (req: Request, res: Response) => {
  try {
    const accountId = req.query.accountId as string;
    const accountNumber = req.query.accountNumber as string;
