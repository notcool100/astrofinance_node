 ...
 *
 * @param accountType The type of account (SB, BB, FD, SH, LS)
 * @returns A unique account number
 */
export const generateAccountNumber = async (
	accountType: string,
): Promise<string> => {
	// Use account type directly as prefix since new types are already prefixes
	const typePrefix = accountType;
