import { PrismaClient, InterestType, AccountType_COA, ExpenseCategoryType, StaffStatus } from '@prisma/client';
...
async function seedRolesAndPermissions() {
  ...
  // Create permissions
  const modules = [
    'users', 'accounts', 'loans', 'accounting', 
    'reports', 'settings', 'admin', 'sms', 
    'tax', 'expenses', 'staff'
  ];
  ...
  const branchManager = await prisma.role.findFirst({ where: { name: 'Branch Manager' } });
  const accountant = await prisma.role.findFirst({ where: { name: 'Accountant' } });
  const loanOfficer = await prisma.role.findFirst({ where: { name: 'Loan Officer' } });
  ...
  const manager = await prisma.adminUser.findFirst({ where: { username: 'manager' } });
  ...
  const accountantUser = await prisma.adminUser.findFirst({ where: { username: 'accountant' } });
  ...
  const loanOfficerUser = await prisma.adminUser.findFirst({ where: { username: 'loan_officer' } });
...
async function seedChartOfAccounts() {
  ...
  const assets = await prisma.account_COA.findFirst({ where: { accountCode: '1000' } });
  const liabilities = await prisma.account_COA.findFirst({ where: { accountCode: '2000' } });
  const equity = await prisma.account_COA.findFirst({ where: { accountCode: '3000' } });
  const income = await prisma.account_COA.findFirst({ where: { accountCode: '4000' } });
  const expenses = await prisma.account_COA.findFirst({ where: { accountCode: '5000' } });
  ...
  if (assets) {
    ...
    const cashAccount = await prisma.account_COA.findFirst({ where: { accountCode: '1100' } });
    if (cashAccount) {
    ...
    const loansAccount = await prisma.account_COA.findFirst({ where: { accountCode: '1200' } });
    if (loansAccount) {
...
async function seedSmsTemplates() {
  ...
  const templates = [
    {
      ...
      category: 'Account',
      content: 'Dear {{name}}, your account {{accountNumber}} has been created successfully. Welcome to AstroFinance!',
    ...
    },
    {
      ...
      content: 'Dear {{name}}, your account {{accountNumber}} has been credited with Rs. {{amount}} on {{date}}. Available balance: Rs. {{balance}}.',
      variables: { name: 'User Name', accountNumber: 'Account Number', amount: 'Amount', date: 'Date', balance: 'Balance' },
      characterCount: 150,
      isActive: true,
    },
    {
      ...
      category: 'Transaction',
      content: 'Dear {{name}}, your account {{accountNumber}} has been debited with Rs. {{amount}} on {{date}}. Available balance: Rs. {{balance}}.',
      variables: { name: 'User Name', accountNumber: 'Account Number', amount: 'Amount', date: 'Date', balance: 'Balance' },
      characterCount: 150,
      isActive: true,
    },
    {
      name: 'Loan Approval',
      category: 'Loan',
      content: 'Dear {{name}}, your loan application {{applicationNumber}} has been approved. Loan amount: Rs. {{amount}}. Please visit our branch for further processing.',
      variables: { name: 'User Name', applicationNumber: 'Application Number', amount: 'Amount' },
      characterCount: 180,
      isActive: true,
    },
    {
      ...
      category: 'Loan',
      content: 'Dear {{name}}, your loan EMI of Rs. {{amount}} for loan {{loanNumber}} is due on {{dueDate}}. Please ensure timely payment to avoid late fees.',
      variables: { name: 'User Name', amount: 'Amount', loanNumber: 'Loan Number', dueDate: 'Due Date' },
      characterCount: 160,
      isActive: true,
    },
    {
      ...
      category: 'Loan',
      content: 'Dear {{name}}, we have received your EMI payment of Rs. {{amount}} for loan {{loanNumber}}. Thank you for your payment.',
      variables: { name: 'User Name', amount: 'Amount', loanNumber: 'Loan Number' },
      characterCount: 130,
      isActive: true,
    }
  ];
  ...
  // Create SMS events
  const accountCreationTemplate = await prisma.smsTemplate.findFirst({ where: { name: 'Account Creation' } });
  const depositTemplate = await prisma.smsTemplate.findFirst({ where: { name: 'Deposit Confirmation' } });
  const withdrawalTemplate = await prisma.smsTemplate.findFirst({ where: { name: 'Withdrawal Confirmation' } });
  const loanApprovalTemplate = await prisma.smsTemplate.findFirst({ where: { name: 'Loan Approval' } });
  const emiDueTemplate = await prisma.smsTemplate.findFirst({ where: { name: 'EMI Due Reminder' } });
  const emiPaymentTemplate = await prisma.smsTemplate.findFirst({ where: { name: 'EMI Payment Confirmation' } });
...
async function seedExpenseCategories() {
  ...
  const personnel = await prisma.expenseCategory.findFirst({ where: { name: 'Personnel Expenses' } });
  const administrative = await prisma.expenseCategory.findFirst({ where: { name: 'Administrative Expenses' } });
  const operational = await prisma.expenseCategory.findFirst({ where: { name: 'Operational Expenses' } });
  const capital = await prisma.expenseCategory.findFirst({ where: { name: 'Capital Expenses' } });
  ...
  const reportTemplates = [
    {
      ...
      queryDefinition: {
        table: 'users',
        joins: [
          { table: 'accounts', on: 'users.id = accounts.userId' }
        ],
        fields: [
          'users.id',
          'users.fullName',
      ...
      layoutDefinition: {
        ...
        columns: [
          ...
          { field: 'balance', header: 'Balance', width: '15%', format: 'currency' },
    ...
    },
    {
      ...
      layoutDefinition: {
        ...
        columns: [
          ...
          { field: 'principalAmount', header: 'Principal', width: '15%', format: 'currency' },
          { field: 'outstandingPrincipal', header: 'Outstanding', width: '15%', format: 'currency' },
          { field: 'status', header: 'Status', width: '10%' },
          { field: 'disbursementDate', header: 'Disbursed On', width: '15%', format: 'date' }
        ],
        footer: 'Generated on {{date}}'
      },
      parameterDefinition: {
    ...
    },
    {
      ...
      queryDefinition: {
        table: 'accounts_coa',
        joins: [
          { table: 'account_balances', on: 'accounts_coa.id = account_balances.accountId' }
        ],
        fields: [
        ...
        groupBy: ['accounts_coa.accountCode', 'accounts_coa.name', 'accounts_coa.accountType'],
      ...
      layoutDefinition: {
        ...
        columns: [
          ...
          { field: 'totalDebit', header: 'Debit', width: '15%', format: 'currency' },
          { field: 'totalCredit', header: 'Credit', width: '15%', format: 'currency' },
          { field: 'balance', header: 'Balance', width: '15%', format: 'currency' }
        ],
        footer: 'Generated on {{date}}'
      },
...
async function seedStaffUsers() {
  ...
  const loanOfficerRole = await prisma.role.findFirst({ where: { name: 'Loan Officer' } });
  const accountantRole = await prisma.role.findFirst({ where: { name: 'Accountant' } });
  
  if (loanOfficerRole) {
    const loanOfficer = await prisma.staff.findFirst({ where: { employeeId: 'STAFF001' } });
    if (loanOfficer) {
  ...
  if (accountantRole) {
    const accountant = await prisma.staff.findFirst({ where: { employeeId: 'STAFF002' } });
    if (accountant) {
      await prisma.staffRole.create({
        data: {
