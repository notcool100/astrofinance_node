...
const createLoanApplicationSchema = (
	selectedLoanType: LoanTypeInterface | null,
) => {
	const baseSchema = yup.object().shape({
		// Step 0: User Selection
		selectedUserId: yup.string().required("Please select a user"),
		...
		amount: yup
			...
			.min(
				selectedLoanType?.minAmount || 1000,
				`Minimum amount is ${selectedLoanType?.minAmount?.toLocaleString() || 1000}`,
			)
			.max(
				selectedLoanType?.maxAmount || 50000,
				`Maximum amount is ${selectedLoanType?.maxAmount?.toLocaleString() || 50000}`,
			),
		tenure: yup
			.number()
			...
			.min(
				selectedLoanType?.minTenure || 3,
				`Minimum tenure is ${selectedLoanType?.minTenure || 3} months`,
			)
			.max(
				selectedLoanType?.maxTenure || 36,
				`Maximum tenure is ${selectedLoanType?.maxTenure || 36} months`,
			),
		purpose: yup
			.string()
		...
		residenceType: yup
			.string()
			.oneOf(
				["OWNED", "RENTED", "FAMILY_OWNED", "COMPANY_PROVIDED", "OTHER"],
...
// Step 0: User Selection Component
const UserSelectionStep: React.FC<{ onNext: () => void }> = ({ onNext }) => {
	const {
		formState: { errors },
		register,
		trigger,
	} = useFormContext<ExtendedLoanApplicationFormData>();
	...
	const {
		...
		error: usersError,
	} = useQuery("users", () => userService.getAllUsers(1, 100), {
	...
	const handleNext = async () => {
		const isValid = await trigger(["selectedUserId"]);
		if (isValid) {
			onNext();
		}
	};
	...
	return (
		<div className="space-y-6">
			...
			<p className="text-sm text-gray-500">
				Choose the user for whom this loan application is being created.
			</p>

			<div className="grid grid-cols-1 gap-6">
				<div>
					<label htmlFor="selectedUserId" className="form-label">
						User <span className="text-red-500">*</span>
					</label>
					<select
						id="selectedUserId"
						className={`form-input ${errors.selectedUserId ? "border-red-300" : ""}`}
					...
					>
						...
						{users.map((user) => (
							<option key={user.id} value={user.id}>
								{user.fullName} ({user.contactNumber}) - {user.userType}
							</option>
						))}
					</select>
					{errors.selectedUserId && (
						<p className="form-error">{errors.selectedUserId.message}</p>
					)}
				</div>
			</div>
...
const LoanDetailsStep: React.FC<{
	...
	const [selectedLoanType, setSelectedLoanType] =
		useState<LoanTypeInterface | null>(null);
	const [emiCalculation, setEmiCalculation] =
		useState<EMICalculationResult | null>(null);
	...
	} = useQuery("loanTypes", () => loanService.getLoanTypes({ active: true }), {
		staleTime: 60 * 60 * 1000, // 1 hour
		retry: 3,
	});
	...
	useEffect(() => {
		if (watchLoanTypeId && loanTypes.length > 0) {
			...
			if (loanType) {
				...
				if (
					!watchAmount ||
					watchAmount < loanType.minAmount ||
					watchAmount > loanType.maxAmount
				) {
				...
				if (
					!watchTenure ||
					watchTenure < loanType.minTenure ||
					watchTenure > loanType.maxTenure
				) {
					...
					>
						...
						{loanTypes.map((loanType) => (
							<option key={loanType.id} value={loanType.id}>
								...
								{loanType.interestType === "FLAT" ? "Flat" : "Reducing"})
							</option>
						))}
					</select>
					{errors.loanTypeId && (
						...
						<div className="mt-2 grid grid-cols-2 gap-4">
							<div>
								...
								<p className="text-sm font-medium text-gray-900">
									₹{emiCalculation.emi.toFixed(2)}
								</p>
							</div>
							<div>
								<p className="text-xs text-gray-500">Interest Rate</p>
								<p className="text-sm font-medium text-gray-900">
									{selectedLoanType.interestRate}% per annum
								</p>
							</div>
							<div>
								<p className="text-xs text-gray-500">Total Amount Payable</p>
								<p className="text-sm font-medium text-gray-900">
									₹{emiCalculation.totalAmount.toFixed(2)}
								</p>
							</div>
							<div>
								<p className="text-xs text-gray-500">Total Interest</p>
								<p className="text-sm font-medium text-gray-900">
									₹{emiCalculation.totalInterest.toFixed(2)}
								</p>
							</div>
						</div>
					) : (
...
const AdditionalInfoStep: React.FC<{ onPrevious: () => void }> = ({
	onPrevious,
}) => {
	const {
		formState: { errors },
				...
				<div className="relative flex items-start">
					...
					<div className="ml-3 text-sm">
						...
						<p className="text-gray-500">
							I agree to the terms and conditions and authorize AstroFinance to
							check my credit history. I confirm that all information provided
...
// Success Component
const SuccessStep: React.FC<{ applicationId: string }> = ({
	applicationId,
}) => {
	return (
		<div className="text-center py-8">
			...
			<div className="mt-2">
				<p className="text-sm text-gray-500">
					Your loan application has been submitted successfully. Your
					application ID is:
				</p>
				...
				<p className="mt-4 text-sm text-gray-500">
					You will be redirected to the application status page shortly. You can
					track the status of your application there.
				</p>
			</div>
			<div className="mt-6">
				...
				<div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
					...
					<div className="bg-white overflow-hidden shadow rounded-lg">
						<div className="px-4 py-5 sm:p-6">
							...
							<p className="mt-2 text-xs text-gray-500">
								If approved, you'll need to upload required documents for
								verification.
							</p>
						</div>
					</div>
					<div className="bg-white overflow-hidden shadow rounded-lg">
						<div className="px-4 py-5 sm:p-6">
							...
							<p className="mt-2 text-xs text-gray-500">
								Once verified, the loan amount will be disbursed to your
								account.
							</p>
						</div>
					</div>
...
const LoanApplicationForm: React.FC<{
	...
	const onSubmit: SubmitHandler<ExtendedLoanApplicationFormData> = async (
		data,
	) => {
		if (!user?.id) {
	...
	return (
		<FormProvider {...methods}>
			<form onSubmit={methods.handleSubmit(onSubmit)} className="space-y-6">
				...
				{currentStep === 1 && <UserSelectionStep onNext={nextStep} />}
				{currentStep === 2 && (
					<LoanDetailsStep onNext={nextStep} onPrevious={prevStep} />
				)}
				{currentStep === 3 && (
					<FinancialInfoStep onNext={nextStep} onPrevious={prevStep} />
				)}
				{currentStep === 4 && <AdditionalInfoStep onPrevious={prevStep} />}
