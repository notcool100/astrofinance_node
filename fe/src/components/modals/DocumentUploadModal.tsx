...
const DocumentUploadModal: React.FC<DocumentUploadModalProps> = ({
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Upload Required Documents">
      <form onSubmit={handleSubmit}>
        <div className="space-y-4">
          <p className="text-sm text-gray-500">
            Please upload the following documents to proceed with your loan application.
            All documents should be in PDF, JPG, or PNG format and less than 5MB in size.
          </p>
            ...
            <div key={doc.id} className="border border-gray-200 rounded-md p-4">
              ...
              {files[doc.id] && (
                <div className="mt-2 text-xs text-gray-500">
                  Selected: {files[doc.id]?.name} ({(files[doc.id]?.size || 0) / 1024 < 1024 
              ...
              {errors[doc.id] && (
                <div className="mt-1 text-xs text-red-500">{errors[doc.id]}</div>
              )}
            </div>
          ))}
        </div>
