...
const AccountForm: React.FC<AccountFormProps> = ({
	...
	const [guardianIdType, setGuardianIdType] = useState(
		account?.bbAccountDetails?.guardianIdType || "NATIONAL_ID",
	);
	...
	const validate = (): boolean => {
		...
		if (typeCode === "BB" && userId) {
			...
			if (guardianContact && !/^\+?[0-9]{10,15}$/.test(guardianContact)) {
				newErrors.guardianContact =
					"Guardian contact must be a valid phone number";
			}
		}
		...
		if (typeCode === "FD" && userId) {
			...
			if (
				depositDay &&
				(parseInt(depositDay) < 1 || parseInt(depositDay) > 31)
			) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			if (isEdit && account) {
				...
				if (account.accountType === "BB" && guardianName) {
					updateData.bbAccountDetails = {
						...
						guardianIdType,
						guardianIdNumber,
						maturityDate: bbMaturityDate
							? bbMaturityDate.toISOString()
							: undefined,
					};

					console.log(
						"Including BB account details in update:",
						updateData.bbAccountDetails,
					);
				}

				// Add MB account details if applicable
				if (account.accountType === "FD" && monthlyDepositAmount) {
					updateData.mbAccountDetails = {
						monthlyDepositAmount: parseFloat(monthlyDepositAmount),
						depositDay: parseInt(depositDay),
						termMonths: parseInt(termMonths),
						maturityDate: mbMaturityDate
							? mbMaturityDate.toISOString()
							: undefined,
					};

					console.log(
						"Including MB account details in update:",
						updateData.mbAccountDetails,
					);
				}
				...
				if (onSuccess) {
					...
					router.push(`users/accounts/${newAccount.id}`);
				}
			}
		} catch (error) {
