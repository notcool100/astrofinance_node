  ...
  const renderAccountRow = (account: Account, level = 0) => {
    ...
    return (
      <React.Fragment key={account.id}>
        <tr className={`${!account.isActive ? 'bg-gray-50 text-gray-500' : ''}`}>
          <td className="px-6 py-4 whitespace-nowrap">
            <div className="flex items-center">
              <div style={{ marginLeft: `${level * 20}px` }} className="flex items-center">
                ...
                <span className={`${!account.isActive ? 'text-gray-500' : 'font-medium text-gray-900'}`}>
                  {account.accountCode}
                </span>
              </div>
            </div>
          ...
          <td className="px-6 py-4 whitespace-nowrap">
            <div className={`${!account.isActive ? 'text-gray-500' : 'text-gray-900'}`}>
              {account.name}
            </div>
          </td>
          <td className="px-6 py-4 whitespace-nowrap">
        ...
        <div className="px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-white shadow rounded-lg overflow-hidden">
            <div className="overflow-x-auto">
              {loading ? (
                <div className="px-6 py-4 text-center text-gray-500">Loading accounts...</div>
              ) : accounts.length === 0 ? (
                <div className="px-6 py-4 text-center text-gray-500">No accounts found</div>
              ) : (
