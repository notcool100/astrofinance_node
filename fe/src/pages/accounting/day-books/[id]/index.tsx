...
import dayBookService, { DayBook, DayBookSummary } from '../../../../services/day-book.service';
...
const DayBookDetailPage: React.FC = () => {
  ...
  const fetchDayBookDetails = async (bookId: string) => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to fetch day book details');
  ...
  const fetchDayBookSummary = async (bookId: string) => {
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to fetch day book summary');
    }
  };
  ...
  const handleCloseDayBook = async () => {
    ...
    if (window.confirm('Are you sure you want to close this day book? This action cannot be undone.')) {
      try {
        await dayBookService.closeDayBook(dayBook.id);
        toast.success('Day book closed successfully');
        fetchDayBookDetails(dayBook.id);
      } catch (error) {
        toast.error(error instanceof Error ? error.message : 'Failed to close day book');
      }
    }
  };
  ...
  const handleReconcileChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
  ...
  const validateReconcileForm = () => {
    ...
    } else if (isNaN(parseFloat(reconcileData.physicalCashBalance)) || parseFloat(reconcileData.physicalCashBalance) < 0) {
      newErrors.physicalCashBalance = 'Physical cash balance must be a positive number';
    }
  ...
  const handleReconcileSubmit = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to reconcile day book');
    }
  };
          ...
          <div className="bg-white shadow overflow-hidden sm:rounded-lg mb-6">
            ...
            <div className="border-t border-gray-200 px-4 py-5 sm:p-0">
              <dl className="sm:divide-y sm:divide-gray-200">
                ...
                <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                  <dt className="text-sm font-medium text-gray-500">System Cash Balance</dt>
                  <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                ...
                {dayBook.isReconciled && (
                  <>
                    <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                      <dt className="text-sm font-medium text-gray-500">Physical Cash Balance</dt>
                      <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                        {formatCurrency(dayBook.physicalCashBalance || 0)}
                      </dd>
                    </div>
                    <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                      <dt className="text-sm font-medium text-gray-500">Discrepancy</dt>
                      <dd className="mt-1 text-sm sm:mt-0 sm:col-span-2">
                        {dayBook.discrepancyAmount !== null && dayBook.discrepancyAmount !== undefined ? (
                          <span className={dayBook.discrepancyAmount < 0 ? 'text-red-600' : 'text-green-600'}>
                    ...
                    {dayBook.discrepancyNotes && (
                      <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt className="text-sm font-medium text-gray-500">Discrepancy Notes</dt>
                        <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                          {dayBook.discrepancyNotes}
                        </dd>
                      </div>
                ...
                {dayBook.isClosed && (
                  <>
                    ...
                    <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                      ...
                      <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                        {dayBook.closedAt ? formatDate(dayBook.closedAt) : 'N/A'}
                      </dd>
                    </div>
                  </>
                )}
              ...
              <div className="px-4 py-5 sm:p-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <p className="mt-1 text-2xl font-semibold text-gray-900">{summary.summary.totalEntries}</p>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h4 className="text-sm font-medium text-gray-500">Total Debits</h4>
                    <p className="mt-1 text-2xl font-semibold text-gray-900">{formatCurrency(summary.summary.totalDebits)}</p>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h4 className="text-sm font-medium text-gray-500">Total Credits</h4>
                    <p className="mt-1 text-2xl font-semibold text-gray-900">{formatCurrency(summary.summary.totalCredits)}</p>
                  </div>
                </div>

                {Object.keys(summary.summary.accountTypeSummary).length > 0 && (
                  <div className="mt-6">
                    <h4 className="text-lg font-medium text-gray-900 mb-4">Account Type Summary</h4>
                    <div className="overflow-x-auto">
                      <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {Object.entries(summary.summary.accountTypeSummary).map(([type, values]) => (
                            <tr key={type}>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {summary.journalEntries.map(entry => {
                      ...
                      const totalAmount = entry.journalEntryLines.reduce(
                        (sum, line) => sum + (parseFloat(line.debitAmount.toString()) > 0 ? parseFloat(line.debitAmount.toString()) : 0),
                        0
                      );
                      ...
                      return (
                        <tr key={entry.id}>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                            <Link href={`/accounting/journal-entries/${entry.id}`} className="text-indigo-600 hover:text-indigo-900">
                              {entry.entryNumber}
                            </Link>
                          </td>
                  ...
                  <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div className="sm:flex sm:items-start">
                      <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                        ...
                        <div className="mb-4">
                          <label htmlFor="physicalCashBalance" className="block text-sm font-medium text-gray-700 mb-1">
                          ...
                          <div className="relative rounded-md shadow-sm">
                            ...
                            <input
                              ...
                              className={`block w-full pl-7 pr-12 border ${
                                reconcileErrors.physicalCashBalance ? 'border-red-300' : 'border-gray-300'
                          ...
                          {reconcileErrors.physicalCashBalance && (
                            <p className="mt-1 text-sm text-red-600">{reconcileErrors.physicalCashBalance}</p>
                          )}
                        </div>

                        <div>
                          <label htmlFor="discrepancyNotes" className="block text-sm font-medium text-gray-700 mb-1">
                            Discrepancy Notes
                          </label>
                          <textarea
                            id="discrepancyNotes"
                          ...
                          <div className="flex">
                            ...
                            <div className="ml-3">
                              ...
                              <div className="mt-2 text-sm text-yellow-700">
                                <p>
                                  System cash balance: {formatCurrency(dayBook.systemCashBalance)}. 
                                  Any difference between physical and system cash will be recorded as a discrepancy.
                                </p>
                              </div>
                            </div>
                          </div>
