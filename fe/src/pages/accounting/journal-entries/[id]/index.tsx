...
import { ArrowLeftIcon, CheckIcon, XMarkIcon, TrashIcon } from '@heroicons/react/24/outline';
import { toast } from 'react-toastify';
import MainLayout from '../../../../components/layout/MainLayout';
import ProtectedRoute from '../../../../components/common/ProtectedRoute';
import Button from '../../../../components/common/Button';
import Badge from '../../../../components/common/Badge';
import journalEntryService, { JournalEntry, JournalEntryStatus } from '../../../../services/journal-entry.service';
...
const JournalEntryDetailPage: React.FC = () => {
  ...
  const fetchJournalEntryDetails = async (entryId: string) => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to fetch journal entry details');
  ...
  const handleApproveJournalEntry = async () => {
    ...
    if (window.confirm('Are you sure you want to post this journal entry?')) {
      try {
        const updatedEntry = await journalEntryService.updateJournalEntryStatus(journalEntry.id, 'POSTED');
  ...
  const handleRejectJournalEntry = async () => {
    ...
    const reason = window.prompt('Please provide a reason for rejecting this journal entry:');
    if (reason) {
      try {
        const updatedEntry = await journalEntryService.updateJournalEntryStatus(journalEntry.id, 'REVERSED', reason);
        setJournalEntry(updatedEntry);
        toast.success('Journal entry rejected successfully');
      } catch (error) {
        toast.error(error instanceof Error ? error.message : 'Failed to reject journal entry');
      }
    }
  };
  ...
  const calculateTotalDebit = () => {
    ...
    return journalEntry.journalEntryLines.reduce((total, line) => total + line.debitAmount, 0);
  };

  const calculateTotalCredit = () => {
    if (!journalEntry) return 0;
    return journalEntry.journalEntryLines.reduce((total, line) => total + line.creditAmount, 0);
  };
                  ...
                  {journalEntry.journalEntryLines.map(line => (
                    <tr key={line.id}>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                        {line.account ? (
                          <Link href={`/accounting/chart-of-accounts/${line.account.id}`} className="text-indigo-600 hover:text-indigo-900">
