...
import { PlusIcon, EyeIcon, TrashIcon, CheckIcon, XMarkIcon } from '@heroicons/react/24/outline';
...
const JournalEntriesPage: React.FC = () => {
  ...
  const fetchJournalEntries = async () => {
    setLoading(true);
    try {
      const { data, pagination: paginationData } = await journalEntryService.getAllJournalEntries({
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to fetch journal entries');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRejectJournalEntry = async (id: string) => {
    ...
    if (reason) {
      try {
        await journalEntryService.updateJournalEntryStatus(id, 'REVERSED', reason);
  ...
  const calculateTotalAmount = (journalEntry: JournalEntry) => {
    return journalEntry.journalEntryLines.reduce((total, line) => {
      return total + (line.debitAmount > 0 ? line.debitAmount : 0);
    }, 0);
  };
        ...
        <div className="px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-white shadow rounded-lg overflow-hidden">
            <div className="overflow-x-auto">
              {loading ? (
                <div className="px-6 py-4 text-center text-gray-500">Loading journal entries...</div>
              ) : journalEntries.length === 0 ? (
                <div className="px-6 py-4 text-center text-gray-500">No journal entries found</div>
              ) : (
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <div className="flex justify-end space-x-2">
                            ...
                            {entry.status === 'DRAFT' && (
                              <>
                                <button
                                  onClick={() => handleApproveJournalEntry(entry.id)}
                                ...
                                <button
                                  onClick={() => handleRejectJournalEntry(entry.id)}
