...
const GeneralLedgerPage: React.FC = () => {
	...
	const [generalLedger, setGeneralLedger] = useState<GeneralLedger | null>(
		null,
	);
	...
	useEffect(() => {
		console.log(
			"generalLedger state changed:",
			generalLedger ? "Data available" : "No data",
		);
	}, [generalLedger]);
	...
	const fetchAccounts = async () => {
		...
		try {
			...
			if (
				...
				console.log(
					"Detected exact format match with",
			...
			// Fallback to other formats
				...
				accountsData = Array.isArray(response.data) ? response.data : [];
				console.log(
					"Using success/data format with",
					accountsData.length,
					"accounts",
				);
			}
	...
	const fetchGeneralLedger = async () => {
		...
		try {
			console.log(
				"Fetching general ledger for period:",
				formattedStartDate,
				"to",
				formattedEndDate,
			);
			...
			const processedData = {
				...
				entries: Array.isArray(data.entries)
					? data.entries.map((entry) => ({
							...
							debitAmount:
								typeof entry.debitAmount === "number" ? entry.debitAmount : 0,
							creditAmount:
								typeof entry.creditAmount === "number" ? entry.creditAmount : 0,
							debitAccounts: Array.isArray(entry.debitAccounts)
								? entry.debitAccounts
								: [],
							creditAccounts: Array.isArray(entry.creditAccounts)
								? entry.creditAccounts
								: [],
						}))
					: [],
			};
					...
					{loading ? (
						...
						<div className="bg-white shadow overflow-hidden sm:rounded-lg">
							<div className="px-4 py-5 sm:px-6 border-b border-gray-200">
								<h3 className="text-lg leading-6 font-medium text-gray-900">
									General Ledger for{" "}
									{generalLedger.period.startDate
										? formatDate(generalLedger.period.startDate)
										: ""}
									to{" "}
									{generalLedger.period.endDate
										? formatDate(generalLedger.period.endDate)
										: ""}
								</h3>
								{generalLedger.account && (
									<p className="mt-1 max-w-2xl text-sm text-gray-500">
										Account: {generalLedger.account.accountCode} -{" "}
								...
								<table className="min-w-full divide-y divide-gray-200">
									...
									<tbody className="bg-white divide-y divide-gray-200">
										{generalLedger.entries.length > 0 ? (
											...
											<tr>
												<td
													colSpan={generalLedger.account ? 5 : 7}
									...
									{generalLedger.entries.length > 0 && (
										<tfoot className="bg-gray-50">
											<tr>
												<th
													scope="row"
													colSpan={generalLedger.account ? 3 : 5}
						...
						<div className="text-center py-8">
							<p className="text-gray-500">
								No general ledger data available. Please generate a report.
							</p>
							<button
								onClick={handleGenerateReport}
