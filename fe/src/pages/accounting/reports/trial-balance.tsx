...
const TrialBalancePage: React.FC = () => {
	...
	const handleExportPDF = async () => {
		...
		try {
			...
			toast.error(
				error instanceof Error
					? error.message
					: "Failed to export trial balance",
			);
		} finally {
	...
	const getAccountBalances = (account: any) => {
		...
		console.log(
			`Processing account: ${account.name}, type: ${accountType}, balance: ${balance}`,
		);
					...
					{loading ? (
								...
								<table className="min-w-full divide-y divide-gray-200">
									...
									<tfoot className="bg-gray-50">
										...
										<tr>
											...
											>
												{formatCurrency(
													Math.abs(trialBalance.totals?.difference || 0),
												)}
												{trialBalance.totals?.difference === 0
													? " (Balanced)"
													: " (Unbalanced)"}
						...
						<div className="text-center py-8">
							<p className="text-gray-500">
								No trial balance data available. Please generate a report.
							</p>
						</div>
					)}
				</div>
