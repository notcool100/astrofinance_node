...
const LoanDocumentsPage = () => {
	...
	const documentTypes: DocumentType[] = [
		...
		},
		{
			...
			name: "Income Proof",
			description:
				"Salary Slips (3 months), Bank Statements (6 months), or Income Tax Returns",
			required: true,
		},
		{
			id: "photo",
		...
		},
		{
			...
			description:
				"Employment ID Card, Appointment Letter, or Business Registration",
			required: false,
		},
	];
	...
	const handleUpload = async () => {
		...
		try {
			...
			formData.append("documentName", uploadedFile.name);
	...
	const handleSubmitAll = async () => {
		...
		if (missingDocuments.length > 0) {
			toast.error(
				`Please upload all required documents: ${missingDocuments.map((d) => d.name).join(", ")}`,
			);
			return;
		}
						...
						<div className="px-4 py-5 sm:p-6">
							...
							<div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
								...
								<div>
									...
									<dd className="mt-1 text-sm text-gray-900">
										{application?.loanType?.name || "Personal Loan"}
									</dd>
								</div>
								<div>
									<dt className="text-sm font-medium text-gray-500">
									...
									<dd className="mt-1 text-sm text-gray-900">
										${application?.amount.toLocaleString() || "10,000"}
									</dd>
								</div>
								<div>
						...
						<div className="px-4 py-5 sm:p-6">
							<div className="flex justify-between items-center mb-6">
								...
								<p className="text-sm text-gray-500">
									Upload all required documents to proceed with your loan
									application
								</p>
							</div>
							...
							<div className="mt-8 flex justify-end">
								<Button
									...
									disabled={documentTypes
										.filter((doc) => doc.required)
										.some((doc) => !getDocumentStatus(doc.id))}
								>
