...
const AccountsPage: React.FC = () => {
  ...
  const fetchAccounts = async () => {
    try {
      setLoading(true);
      const response = await getAllAccounts(page, limit, search, accountType, status);
      ...
      Cell: ({ value }: { value: string }) => (
        <Badge
          color={
            value === 'SAVINGS' ? 'blue' :
            value === 'LOAN' ? 'red' :
            value === 'FIXED_DEPOSIT' ? 'green' : 'gray'
          }
          text={
            value === 'SAVINGS' ? 'Savings' :
            value === 'LOAN' ? 'Loan' :
            value === 'FIXED_DEPOSIT' ? 'Fixed Deposit' : value
          }
        />
      ),
    },
          ...
          <div className="sm:flex sm:items-center">
            <div className="sm:flex-auto">
              ...
              <p className="mt-2 text-sm text-gray-700">
                A list of all accounts in the system including their account number, type, balance, and status.
              </p>
            </div>
