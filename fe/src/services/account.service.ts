  ...
  async getAccounts(): Promise<any> {
    try {
      ...
      if (response.data && response.data.success === true && Array.isArray(response.data.data)) {
        console.log('Detected success/data format with', response.data.data.length, 'accounts');
        return response.data;
      } else if (response.data && Array.isArray(response.data)) {
        // API returns array directly
        console.log('Detected direct array format with', response.data.length, 'accounts');
        return { data: response.data };
      } else if (typeof response.data === 'object') {
        // Try to extract any array we can find
        for (const key in response.data) {
          if (Array.isArray(response.data[key])) {
            console.log('Found array in response at key:', key, 'with', response.data[key].length, 'items');
            return { data: response.data[key] };
          }
        }
        // If we can't find an array, return an empty array
        console.warn('Could not find any array in response, returning empty array');
        return { data: [] };
      } else {
        // Fallback
