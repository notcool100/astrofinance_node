...
class FinancialReportService {
  ...
  async getIncomeStatement(startDate?: string, endDate?: string): Promise<IncomeStatement> {
    ...
    try {
      // Make a direct axios call to ensure we get the raw response
      const response = await axios.get(`${API_URL}/accounting/reports/income-statement?${params.toString()}`, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  async getBalanceSheet(asOfDate?: string): Promise<BalanceSheet> {
    ...
    try {
      // Make a direct axios call to ensure we get the raw response
      const response = await axios.get(`${API_URL}/accounting/reports/balance-sheet?${params.toString()}`, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  async getGeneralLedger(
    ...
    try {
      // Make a direct axios call to ensure we get the raw response
      const response = await axios.get(`${API_URL}/accounting/reports/general-ledger?${params.toString()}`, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        },
   ...
   */
  async exportReport(
    reportType: 'balance-sheet' | 'income-statement' | 'trial-balance' | 'general-ledger',
    format: 'pdf' | 'excel',
